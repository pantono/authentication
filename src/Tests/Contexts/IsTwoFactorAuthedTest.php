...
class IsTwoFactorAuthedTest extends TestCase
{
    ...
    {
        $validator = new IsTwoFactorAuthed($this->userAuthentication, $this->securityContext);
    ...
    public function testInvalidAuthNoSession()
    {
        ...
        $this->expectExceptionMessage('Secondary auth required to complete this request');
    ...
    public function testInvalidAuthCodeNotExists()
    {
        ...
        $session->expects($this->once())
            ...
            ->willReturn('test');
        $this->expectException(TwoFactorAuthRequired::class);
        $this->expectExceptionMessage('Please re-verify your two factor auth for this request');
        $this->userAuthentication->expects($this->once())
            ->method('isTwoFactorCodeValid')
//            ->with('test', new \DateTime('2022-01-01 00:00:00'))
            ->willReturn(false);
        $validator->isValid($request, $endpoint, new ParameterBag(), $session);
    }
